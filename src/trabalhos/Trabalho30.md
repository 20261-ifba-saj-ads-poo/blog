...
2. **Desconto por Inadimplência** – Caso o colaborador esteja em débito relativo a benefícios, aplica desconto de 5% do salário bruto.
...
TelaJornada -> ServicoPagamento : calcularPagamento(folhaPagamento)
ServicoPagamento -> ServicoPagamento : validarHorasExtras(horas)
ServicoPagamento -> ServicoPagamento : aplicarDescontoInadimplencia()
ServicoPagamento -> ServicoPagamento : aplicarBonusTempoServico()
alt tudo válido
    ServicoPagamento -> RepositorioFolha : salvarFolha()
    RepositorioFolha --> ServicoPagamento : sucesso
    ServicoPagamento --> TelaJornada : folha gerada
    TelaJornada --> Usuario : exibir folha
else erro
    ServicoPagamento --> TelaJornada : mensagem de erro
...
| Dados | 20 pts | Estrutura adequada de armazenamento em memória |
